...
class RunoutHelper:
    ...
    # The fork reports where a motion sensor last saw the filament and where
    # it will call a runout (filament_position / runout_position). Those are
    # recorded through their own method, so note_filament_present keeps
    # upstream's (eventtime, is_filament_present) shape. Other extras depend
    # on that shape: AFC wraps note_filament_present and picks its call
    # convention from the parameter list -- given two more parameters it
    # picks Kalico's and fails on the first sensor edge.
    def note_runout_position(self, extruder_pos, runout_pos):
    ...
    def note_filament_present(self, eventtime, is_filament_present):
        ...
        if is_filament_present:
            ...
            if (self.last_extruder_pos is not None
                    and self.last_runout_pos is not None):
                pos_info = ", Extruder Position: %.2f mm, Runout Position: %.2f mm" % (self.last_extruder_pos, self.last_runout_pos)
            logging.info(
